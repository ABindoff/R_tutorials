...
df$t <- factor(df$t, labels = c("5mo", "20mo")) # make t a factor with sensible labels
...
Alternatively, we could change the data frame (and while we're at it, re-order 'wt' as the baseline)  
...
```{r}
ggplot(df, aes(x = strain, y = m, fill = t)) +
...
```{r}
p <- ggplot(df0, aes(x = strain, y = mean.m, colour = t)) +
...
```{r}
p + scale_colour_manual(values = c("dodgerblue4", "firebrick"), name = element_blank(), labels = c("5 months old", "20 months old")) +
